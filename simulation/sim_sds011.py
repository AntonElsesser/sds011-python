...
class SimulationSDS011:
    ...
    def command_message_valid(self) -> bool:
        if len(self.command) == 19:
            if self.command[0] == Frame.HEADER.value and self.command[1] == MessageType.COMMAND.value and self.command[-1] == Frame.TAIL.value:
                message_data = []
                ...
                if SDS011.calculate_checksum(message_data) == self.command[-2]:
                    if self.command[-4:-2] == bytes([255, 255]) or self.command[-4:-2] == bytes(self.device_id):
                        return True                   
        return False
